...
export const updateWatchProgress = async (
  ...
  try {
    ...
    if (progress === undefined || typeof progress !== "number") {
      return res
        .status(400)
        .json({ message: "Progress must be provided as a number" });
    }

    const watchHistoryEntry = await withPrismaErrorHandling(async () => {
...
export const getWatchHistory = async (
  ...
  try {
    ...
    const watchHistory = await withPrismaErrorHandling(async () => {
